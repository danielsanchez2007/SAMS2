    ...
    <div class="tema-modal-opaco bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md border border-slate-700/60" @click.stop>
        ...
        <div class="p-6 sm:p-8">
            <h3 class="text-2xl font-bold text-slate-100 mb-6" 
                x-text="formUso.id ? 'Editar Uso' : 'Nuevo Uso'">
            </h3>

            <p class="text-slate-400 text-sm mb-6" x-show="!formUso.id">
                Define el uso que se le da al item/equipo (ejemplo: Mantenimiento, Inspección, Transporte, Almacenaje, etc.)
            </p>

            <form :action="formUso.id ? '{{ url('uso-items/uso') }}/' + formUso.id : '{{ route('uso-items.store.uso') }}'" 
                  method="POST" 
                  class="space-y-6">

                @csrf
                <template x-if="formUso.id">
                    <input type="hidden" name="_method" value="PUT">
                </template>

                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">
                        Nombre del Uso <span class="text-rose-400">*</span>
                    </label>
                    <input 
                        type="text" 
                        name="nombre" 
                        x-model="formUso.nombre" 
                        required 
                        placeholder="Ej: Mantenimiento preventivo, Inspección diaria, Transporte interno, Uso en campo, Almacenaje prolongado..." 
