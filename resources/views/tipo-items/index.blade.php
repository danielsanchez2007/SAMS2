...
@section('title', 'Tipo de Items')

@section('content')

<div class="space-y-8" x-data="tipoItemsApp()">

    <!-- Barra: filtro automático, tamaño tabla, acciones -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <form x-ref="filterForm" method="GET" action="{{ route('tipo-items.index') }}" class="flex flex-wrap items-center gap-3">
            <label class="text-sm font-medium text-slate-400">Buscar:</label>
            <div class="relative">
                <input type="text" name="q" value="{{ $search ?? '' }}" placeholder="Nombre del tipo de item..."
                ...
                @if(!empty($search))
                <a href="{{ route('tipo-items.index', ['per_page' => $perPage ?? 10]) }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300" title="Limpiar">✕</a>
                @endif
            </div>
    ...
    <div class="bg-slate-900/70 backdrop-blur-md rounded-2xl border-2 tema-table-wrap shadow-2xl overflow-hidden">
                    ...
                    @forelse($tipoItems as $i => $t)
                        <tr class="hover:bg-slate-800/50 transition-colors duration-200">
                            ...
                            <td class="py-4 px-8 text-right flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('tipo-items.destroy', $t) }}" method="POST" class="inline" onsubmit="return confirm('¿Realmente deseas eliminar este tipo de item? Esta acción no se puede deshacer.');">
                                    @csrf
                                    @method('DELETE')
                                    <button 
                                        type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="py-16 px-6 text-center text-slate-500 text-lg italic">
                                No hay tipos de items registrados todavía.<br>
        ...
        <!-- Paginación -->
        @if(method_exists($tipoItems, 'links'))
        <div class="px-6 py-4 border-t border-slate-800/60 bg-slate-900/50">
            {{ $tipoItems->links('pagination::tailwind') }}
        </div>
        @endif
    </div>
    ...
    <div x-show="showExportModal" 
        ...
        <div class="tema-modal-export tema-modal-content rounded-2xl w-full max-w-4xl my-8 p-8" @click.stop>
            ...
            <div class="bg-white rounded-xl p-6 mb-8 border border-gray-200 max-h-[65vh] overflow-y-auto">
                ...
                <p class="text-lg font-semibold text-indigo-300 mb-5">Lista de tipos de items</p>

                <table class="w-full text-sm border-collapse">
                    <thead>
                        <tr class="bg-slate-800/80 text-slate-200 uppercase text-xs tracking-wider">
                            <th class="border border-slate-700 p-3 text-left">Nombre del Tipo</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-800">
                        @forelse($tipoItems as $i => $t)
                            <tr class="hover:bg-slate-900/40 transition-colors">
                                <td class="border border-slate-800 p-3 font-medium">{{ $t->nombre }}</td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <a href="{{ route('tipo-items.export.excel') }}" 
                ...
                <a href="{{ route('tipo-items.export.pdf') }}" 
    ...
    <!-- Modal de Crear/Editar Tipo de Item -->
    @include('tipo-items.modals.tipo')

</div>

<!-- Script Alpine (sin cambios) -->
<script>
function tipoItemsApp() {
    return {
        ...
        editTipo(t) {
            this.formTipo = { id: t.id, nombre: t.nombre };
            this.modalTipo = true;
        }
    };
}
