...
<div class="space-y-8" x-data="cargosApp()">

    <!-- Barra: filtro automático, tamaño tabla, acciones -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <form x-ref="filterForm" method="GET" action="{{ route('cargos.index') }}" class="flex flex-wrap items-center gap-3">
            <label class="text-sm font-medium text-slate-400">Buscar:</label>
            <div class="relative">
                <input type="text" name="q" value="{{ $search ?? '' }}" placeholder="Nombre del cargo..."
                ...
                @if(!empty($search))
                <a href="{{ route('cargos.index', ['per_page' => $perPage ?? 10]) }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300" title="Limpiar">✕</a>
                @endif
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-slate-300 min-w-[600px]">
                <thead class="bg-slate-800/90">
                    <tr>
                        <th class="text-left py-4 px-6 font-semibold text-slate-400 uppercase tracking-wider text-xs">Nombre del Cargo</th>
                ...
                <tbody class="divide-y divide-slate-800/40">
                    @forelse($cargos as $i => $c)
                        <tr class="hover:bg-slate-800/50 transition-colors duration-200">
                            <td class="py-4 px-6 font-medium text-slate-100">{{ $c->nombre }}</td>
                            <td class="py-4 px-8 text-right flex items-center justify-end gap-2">
                                <button 
                                    type="button" 
                                    @click="editCargo({{ json_encode($c) }})" 
                                ...
                                <form action="{{ route('cargos.destroy', $c) }}" method="POST" class="inline" onsubmit="return confirm('¿Realmente deseas eliminar este cargo? Esta acción no se puede deshacer.');">
                                    @csrf
                                    @method('DELETE')
                                    <button 
                                        type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="py-16 px-6 text-center text-slate-500 text-lg italic">
                                No hay cargos registrados todavía.<br>
                                <span class="text-slate-400">Haz clic en "Agregar Cargo" para comenzar.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div x-show="showExportModal" 
        ...
        <div class="tema-modal-export tema-modal-content rounded-2xl w-full max-w-4xl my-8 p-8" @click.stop>
            ...
            <div class="bg-white rounded-xl p-6 mb-8 border border-gray-200 max-h-[65vh] overflow-y-auto">
                ...
                <p class="text-lg font-semibold mb-5" style="color: var(--tema-primary);">Lista de cargos</p>
                ...
                <table class="w-full text-sm border-collapse">
                    <thead>
                        <tr class="text-white uppercase text-xs tracking-wider" style="background: linear-gradient(135deg, var(--tema-from), var(--tema-to));">
                            <th class="border border-white/30 p-3 text-left">Nombre del Cargo</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @forelse($cargos as $i => $c)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="border border-gray-200 p-3 font-medium text-gray-900">{{ $c->nombre }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="2" class="border border-gray-200 p-6 text-center text-gray-500">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <a href="{{ route('cargos.export.excel') }}" 
                ...
                <a href="{{ route('cargos.export.pdf') }}" 
                   class="flex-1 inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl tema-gradient text-white font-semibold transition-all shadow-md hover:opacity-90">
                    Descargar PDF
                </a>
            </div>
...
function cargosApp() {
    return {
        ...
        modalCargo: false,
        formCargo: { id: null, nombre: '' },
        openModalCargo() {
            this.formCargo = { id: null, nombre: '' };
            this.modalCargo = true;
        },
        editCargo(c) {
            this.formCargo = { id: c.id, nombre: c.nombre };
            this.modalCargo = true;
        }
    };
}
