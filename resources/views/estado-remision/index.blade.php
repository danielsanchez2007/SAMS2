...
@section('title', 'Estado de Remisión')

@section('content')

<div class="space-y-8" x-data="estadoRemisionApp()">

    <!-- Barra: filtro automático, tamaño tabla, acciones -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <form x-ref="filterForm" method="GET" action="{{ route('estado-remision.index') }}" class="flex flex-wrap items-center gap-3">
            <label class="text-sm font-medium text-slate-400">Buscar:</label>
            <div class="relative">
                <input type="text" name="q" value="{{ $search ?? '' }}" placeholder="Nombre del estado..."
                ...
                @if(!empty($search))
                <a href="{{ route('estado-remision.index', ['per_page' => $perPage ?? 10]) }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300" title="Limpiar">✕</a>
                @endif
            </div>
    ...
    <div class="bg-slate-900/70 backdrop-blur-md rounded-2xl border-2 tema-table-wrap shadow-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-slate-300 min-w-[600px]">
                <thead class="bg-slate-800/90">
                    <tr>
                        <th class="text-left py-4 px-6 font-semibold text-slate-400 uppercase tracking-wider text-xs">Nombre del Estado</th>
                        <th class="text-right py-4 px-8 font-semibold text-slate-400 uppercase tracking-wider text-xs w-32">Acciones</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-800/40">
                    @forelse($estadoRemisiones as $i => $e)
                        <tr class="hover:bg-slate-800/50 transition-colors duration-200">
                            ...
                            <td class="py-4 px-8 text-right flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('estado-remision.destroy', $e) }}" method="POST" class="inline" onsubmit="return confirm('¿Realmente deseas eliminar este estado de remisión? Esta acción no se puede deshacer.');">
                                    @csrf
                                    @method('DELETE')
                                    <button 
                                        type="submit" 
                                        class="p-2.5 text-rose-400 hover:text-rose-300 hover:bg-rose-950/30 rounded-lg transition-all duration-200"
                                        title="Eliminar estado de remisión"
                                    >
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="py-16 px-6 text-center text-slate-500 text-lg italic">
                                No hay estados de remisión registrados todavía.<br>
                                <span class="text-slate-400">Haz clic en "Agregar Estado" para comenzar.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if(method_exists($estadoRemisiones, 'links'))
        <div class="px-6 py-4 border-t border-slate-800/60 bg-slate-900/50">
            {{ $estadoRemisiones->links('pagination::tailwind') }}
        </div>
        @endif
    </div>
    ...
    <div x-show="showExportModal" 
        ...
        <div class="tema-modal-export tema-modal-content rounded-2xl w-full max-w-4xl my-8 p-8" @click.stop>
            ...
            <div class="bg-white rounded-xl p-6 mb-8 border border-gray-200 max-h-[65vh] overflow-y-auto">
                ...
                <p class="text-lg font-semibold mb-5" style="color: var(--tema-primary);">Lista de estados de remisión</p>

                <table class="w-full text-sm border-collapse">
                    <thead>
                        <tr class="bg-slate-800/80 text-slate-200 uppercase text-xs tracking-wider">
                            <th class="border border-slate-700 p-3 text-left">Nombre del Estado</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-800">
                        @forelse($estadoRemisiones as $i => $e)
                            <tr class="hover:bg-slate-900/40 transition-colors">
                                <td class="border border-slate-800 p-3 font-medium">{{ $e->nombre }}</td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <a href="{{ route('estado-remision.export.excel') }}" 
                ...
                <a href="{{ route('estado-remision.export.pdf') }}" 
                   class="flex-1 inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl tema-gradient text-white font-semibold transition-all shadow-md hover:opacity-90">
                    Descargar PDF
                </a>
            </div>
    ...
    <!-- Modal de Crear/Editar Estado de Remisión -->
    @include('estado-remision.modals.remision')

</div>

<!-- Script Alpine (sin cambios) -->
<script>
function estadoRemisionApp() {
    return {
        showExportModal: false,
        modalRemision: false,
        formRemision: { id: null, nombre: '' },
        openModalRemision() {
            this.formRemision = { id: null, nombre: '' };
            this.modalRemision = true;
        },
        editRemision(e) {
            this.formRemision = { id: e.id, nombre: e.nombre };
            this.modalRemision = true;
        }
    };
}
