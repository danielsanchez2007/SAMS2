    ...
    <div class="tema-modal-opaco bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md border border-slate-700/60" @click.stop>
        ...
        <div class="p-6 sm:p-8">
            <h3 class="text-2xl font-bold text-slate-100 mb-6" 
                x-text="formRemision.id ? 'Editar Estado de Remisión' : 'Nuevo Estado de Remisión'">
            </h3>

            <form :action="formRemision.id ? '{{ url('estado-remision') }}/' + formRemision.id : '{{ route('estado-remision.store') }}'" 
                  method="POST" 
                  class="space-y-6">

                @csrf
                <template x-if="formRemision.id">
                    <input type="hidden" name="_method" value="PUT">
                </template>

                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">
                        Nombre del Estado <span class="text-rose-400">*</span>
                    </label>
                    <input 
                        type="text" 
                        name="nombre" 
                        x-model="formRemision.nombre" 
                        required 
                        placeholder="Ej: Pendiente, En tránsito, Recibido, Rechazado, Anulado..." 
