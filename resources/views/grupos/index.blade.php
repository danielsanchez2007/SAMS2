...
<div class="space-y-8" x-data="gruposApp()">

    <!-- Barra: filtro automático, tamaño tabla, acciones -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <form x-ref="filterForm" method="GET" action="{{ route('grupos.index') }}" class="flex flex-wrap items-center gap-3">
            <label class="text-sm font-medium text-slate-400">Buscar:</label>
            <div class="relative">
                <input type="text" name="q" value="{{ $search ?? '' }}" placeholder="Nombre del grupo..."
                ...
                @if(!empty($search))
                <a href="{{ route('grupos.index', ['per_page' => $perPage ?? 10]) }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300" title="Limpiar">✕</a>
                @endif
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-slate-300 min-w-[600px]">
                <thead class="bg-slate-800/90">
                    <tr>
                        <th class="text-left py-4 px-6 font-semibold text-slate-400 uppercase tracking-wider text-xs">Nombre del Grupo</th>
                ...
                <tbody class="divide-y divide-slate-800/40">
                    @forelse($grupos as $i => $g)
                        <tr class="hover:bg-slate-800/50 transition-colors duration-200">
                            <td class="py-4 px-6 font-medium text-slate-100">{{ $g->nombre }}</td>
                            <td class="py-4 px-8 text-right flex items-center justify-end gap-2">
                                <button 
                                    type="button" 
                                    @click="editGrupo({{ json_encode($g) }})" 
                                ...
                                <form action="{{ route('grupos.destroy', $g) }}" method="POST" class="inline" onsubmit="return confirm('¿Realmente deseas eliminar este grupo? Esta acción no se puede deshacer.');">
                                    @csrf
                                    @method('DELETE')
                                    <button 
                                        type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="py-16 px-6 text-center text-slate-500 text-lg italic">
                                No hay grupos registrados todavía.<br>
                                <span class="text-slate-400">Haz clic en "Agregar Grupo" para comenzar.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div x-show="showExportModal" 
        ...
        <div class="tema-modal-export tema-modal-content rounded-2xl w-full max-w-4xl my-8 p-8" @click.stop>
            ...
            <div class="bg-white rounded-xl p-6 mb-8 border border-gray-200 max-h-[65vh] overflow-y-auto">
                ...
                <p class="text-lg font-semibold mb-5" style="color: var(--tema-primary);">Lista de grupos</p>

                <table class="w-full text-sm border-collapse">
                    <thead>
                        <tr class="bg-slate-800/80 text-slate-200 uppercase text-xs tracking-wider">
                            <th class="border border-slate-700 p-3 text-left">Nombre del Grupo</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-800">
                        @forelse($grupos as $i => $g)
                            <tr class="hover:bg-slate-900/40 transition-colors">
                                <td class="border border-slate-800 p-3 font-medium">{{ $g->nombre }}</td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <a href="{{ route('grupos.export.excel') }}" 
                ...
                <a href="{{ route('grupos.export.pdf') }}" 
                   class="flex-1 inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl tema-gradient text-white font-semibold transition-all shadow-md hover:opacity-90">
                    Descargar PDF
                </a>
            </div>
...
function gruposApp() {
    return {
        ...
        modalGrupo: false,
        formGrupo: { id: null, nombre: '' },
        openModalGrupo() {
            this.formGrupo = { id: null, nombre: '' };
            this.modalGrupo = true;
        },
        editGrupo(g) {
            this.formGrupo = { id: g.id, nombre: g.nombre };
            this.modalGrupo = true;
        }
    };
}
