    ...
    <div class="tema-modal-opaco bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md border border-slate-700/60" @click.stop>
        ...
        <div class="p-6 sm:p-8">
            <h3 class="text-2xl font-bold text-slate-100 mb-6" 
                x-text="formGrupo.id ? 'Editar Grupo' : 'Nuevo Grupo'">
            </h3>

            <form :action="formGrupo.id ? '{{ url('grupos') }}/' + formGrupo.id : '{{ route('grupos.store') }}'" 
                  method="POST" 
                  class="space-y-6">

                @csrf
                <template x-if="formGrupo.id">
                    <input type="hidden" name="_method" value="PUT">
                </template>

                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">
                        Nombre del Grupo <span class="text-rose-400">*</span>
                    </label>
                    <input 
                        type="text" 
                        name="nombre" 
                        x-model="formGrupo.nombre" 
                        required 
                        placeholder="Ej: Seguridad Industrial, Prevención de Riesgos, Administrativo..." 
