    ...
    <div class="tema-modal-opaco bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg border border-slate-700/60" @click.stop>
        ...
        <div class="p-6 sm:p-8">
            <h3 class="text-2xl font-bold text-slate-100 mb-6" 
                x-text="formBodega.id ? 'Editar Bodega' : 'Nueva Bodega'">
            </h3>

            <p class="text-slate-400 text-sm mb-6">
                La ubicación (país, departamento, municipio) se hereda automáticamente de la sede seleccionada.
            </p>

            <form :action="formBodega.id ? '{{ url('empresas/bodega') }}/' + formBodega.id : '{{ route('empresas.store.bodega') }}'" 
                  method="POST" 
                  class="space-y-6">

                @csrf
                <template x-if="formBodega.id">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">
                        Sede <span class="text-rose-400">*</span>
                    </label>
                    <select 
                        name="sede_id" 
                        x-model="formBodega.sede_id" 
                        required
                    ...
                    >
                        ...
                        @foreach($allSedes as $sede)
                            <option value="{{ $sede->id }}">
                                {{ $sede->nombre }} 
                                ({{ $sede->empresa->nombre ?? 'Sin empresa' }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">
                        Nombre de la Bodega <span class="text-rose-400">*</span>
                    </label>
                    <input 
                        type="text" 
                        name="nombre" 
                        x-model="formBodega.nombre" 
                        required 
                        placeholder="Ej: Bodega Principal, Almacén Central, Zona de Herramientas, Bodega Externa..." 
