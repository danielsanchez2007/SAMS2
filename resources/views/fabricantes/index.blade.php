...
<div class="space-y-8" x-data="{ showModal: false, showExportModal: false, formData: { id: null, nombre: '' } }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <form x-ref="filterForm" method="GET" action="{{ route('fabricantes.index') }}" class="flex flex-wrap items-center gap-3">
            <label class="text-sm font-medium text-slate-400">Buscar:</label>
            <div class="relative">
                <input type="text" name="q" value="{{ $search ?? '' }}" placeholder="Nombre del fabricante..."
                ...
                @if(!empty($search))
                <a href="{{ route('fabricantes.index', ['per_page' => $perPage ?? 10]) }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300" title="Limpiar">✕</a>
                @endif
            </div>
    ...
    <div class="bg-slate-900/70 backdrop-blur-md rounded-2xl border-2 tema-table-wrap shadow-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-slate-300 min-w-[600px]">
                <thead class="bg-slate-800/90">
                    <tr>
                        <th class="text-left py-4 px-6 font-semibold text-slate-400 uppercase tracking-wider text-xs">Nombre del Fabricante</th>
                        <th class="text-right py-4 px-8 font-semibold text-slate-400 uppercase tracking-wider text-xs w-32">Acciones</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-800/40">
                    @forelse($fabricantes as $i => $f)
                        <tr class="hover:bg-slate-800/50 transition-colors duration-200">
                            <td class="py-4 px-6 font-medium text-slate-100">{{ $f->nombre }}</td>
                            <td class="py-4 px-8 text-right flex items-center justify-end gap-2">
                                <button type="button" 
                                        @click="formData = { id: {{ $f->id }}, nombre: '{{ addslashes($f->nombre) }}' }; showModal = true" 
                                ...
                                <form action="{{ route('fabricantes.destroy', $f) }}" method="POST" class="inline" onsubmit="return confirm('¿Realmente deseas eliminar este fabricante?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="py-16 px-6 text-center text-slate-500 text-lg italic">
                                No hay fabricantes registrados todavía.<br>
                                <span class="text-slate-400">Haz clic en "Agregar Fabricante" para comenzar.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if(method_exists($fabricantes, 'hasPages') && ($fabricantes->hasPages() || $fabricantes->total() > 0))
        <div class="flex flex-wrap items-center justify-between gap-4 px-6 py-4 border-t border-slate-800/60 bg-slate-900/50">
            <p class="text-sm text-slate-400">Mostrando {{ $fabricantes->firstItem() ?? 0 }} a {{ $fabricantes->lastItem() ?? 0 }} de {{ $fabricantes->total() }} fabricantes</p>
            <div class="flex items-center gap-2">
                @if ($fabricantes->onFirstPage())
                    <span class="px-3 py-2 rounded-lg border border-slate-700 text-sm text-slate-500 cursor-not-allowed">Anterior</span>
                @else
                    <a href="{{ $fabricantes->previousPageUrl() }}" class="px-3 py-2 rounded-lg border border-slate-700 text-sm font-medium text-slate-300 hover:bg-slate-800 transition-colors">Anterior</a>
                @endif
                <span class="text-sm text-slate-400 px-2">Página {{ $fabricantes->currentPage() }} de {{ $fabricantes->lastPage() }}</span>
                @if ($fabricantes->hasMorePages())
                    <a href="{{ $fabricantes->nextPageUrl() }}" class="px-3 py-2 rounded-lg border border-slate-700 text-sm font-medium text-slate-300 hover:bg-slate-800 transition-colors">Siguiente</a>
                @else
    ...
    <!-- Modal Agregar/Editar Fabricante -->
        ...
        <div class="bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md border border-slate-700/60 backdrop-blur-md" @click.stop>
            ...
            <div class="p-6 sm:p-8">
                <h3 class="text-2xl font-bold text-slate-100 mb-6" 
                    x-text="formData.id ? 'Editar Fabricante' : 'Nuevo Fabricante'">
                </h3>

                <form :action="formData.id ? '{{ url('fabricantes') }}/' + formData.id : '{{ route('fabricantes.store') }}'" 
                      method="POST" 
                      class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">
                            Nombre del Fabricante <span class="text-rose-400">*</span>
                        </label>
                        <input 
                            type="text" 
                            name="nombre" 
                            ...
                            required 
                            placeholder="Ej: 3M, Honeywell, Stanley Black & Decker, Bosch, DeWalt..." 
    ...
    <div x-show="showExportModal" 
        ...
        <div class="tema-modal-opaco tema-modal-export tema-modal-content bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl my-8 p-8 border border-slate-700/60" @click.stop>
            ...
            <div class="bg-slate-950/60 rounded-xl p-6 mb-8 border border-slate-800/50 max-h-[65vh] overflow-y-auto">
                ...
                <p class="text-lg font-semibold text-indigo-300 mb-5">Lista de fabricantes</p>

                <table class="w-full text-sm border-collapse">
                    <thead>
                        <tr class="bg-slate-800/80 text-slate-200 uppercase text-xs tracking-wider">
                            <th class="border border-slate-700 p-3 text-left">Nombre del Fabricante</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-800">
                        @forelse($fabricantes as $i => $f)
                            <tr class="hover:bg-slate-900/40 transition-colors">
                                <td class="border border-slate-800 p-3 font-medium">{{ $f->nombre }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="2" class="border border-slate-800 p-6 text-center text-slate-500">
                                    No hay fabricantes para mostrar
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <a href="{{ route('fabricantes.export.excel') }}" 
                ...
                <a href="{{ route('fabricantes.export.pdf') }}" 
